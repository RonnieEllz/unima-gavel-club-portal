  ...
  do $$
  begin
    create type admin_role as enum ('super_admin', 'administrator', 'operations_admin', 'treasurer', 'content_administrator');
  exception
    when duplicate_object then null;
  end $$;
  ...
  create or replace function set_member_payment_status(
  ...
  begin
    if not can_manage_payments() then
  ...
  create or replace function get_payment_members(
  ...
  revoke all on function set_member_payment_status(uuid, boolean) from public;
  grant execute on function set_member_payment_status(uuid, boolean) to authenticated;
  create or replace function set_members_payment_status(
  ...
  begin
    ...
    if exists (
      select 1 from unnest(target_member_ids) requested(id)
      left join profiles p on p.id = requested.id
      where p.id is null
    ) then
    ...
    with previous as (
      select p.id,
        jsonb_build_object('payment_verified', p.payment_verified, 'last_payment_date', p.last_payment_date) as before_data
      from profiles p
      where p.id = any(target_member_ids)
      for update
    ), updated as (
      update profiles p
      set payment_verified = paid, last_payment_date = next_payment_date
      from previous
      where p.id = previous.id
      returning p.id, previous.before_data
    )
    select updated.id, updated.before_data,
      jsonb_build_object('payment_verified', paid, 'last_payment_date', next_payment_date)
    from updated;
  end;
  $$ language plpgsql security definer set search_path = public;
  revoke all on function set_members_payment_status(uuid[], boolean) from public;
  grant execute on function set_members_payment_status(uuid[], boolean) to authenticated;
  revoke all on function get_payment_members(text, boolean) from public;
  grant execute on function get_payment_members(text, boolean) to authenticated;
